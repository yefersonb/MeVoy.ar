...
const VehiculosConductor = () => {
  ...
  const agregarVehiculo = async () => {
    ...
    try {
      ...
      console.error('Error aÃ±adiendo vehÃ­culo:', e);
      alert('No se pudo agregar el vehÃ­culo.');
    } finally {
  ...
  const saveEdit = async (veh) => {
    ...
    try {
      ...
      console.error('Error guardando ediciÃ³n:', e);
  ...
  const eliminarVehiculo = async (vehId) => {
    if (!usuario) return;
    const ok = window.confirm('QuerÃ©s eliminar ese vehÃ­culo? Esta acciÃ³n no se puede deshacer.');
    if (!ok) return;
    try {
      ...
      alert('No se pudo eliminar el vehÃ­culo.');
    }
  };
  ...
  return (
    <div style={{ marginTop: 28, fontFamily: 'inherit' }}>
      <div style={{ marginBottom: 24, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        <SubTab active={activeSubTab === 'agregar'} label="Agregar vehÃ­culo" onClick={() => setActiveSubTab('agregar')} />
        <SubTab
          active={activeSubTab === 'gestionar'}
          label="Modificar / Eliminar vehÃ­culo"
      ...
      {activeSubTab === 'agregar' && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: colors.primary }}>Agregar vehÃ­culo</div>
            <div style={{ fontSize: 13, color: '#555' }}>CompletÃ¡ los datos para registrar un nuevo vehÃ­culo.</div>
          </div>
          ...
          >
              ...
              <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 150px' }}>
                  ...
                  {uploadStatusNuevo.cedulaFile === 'error' && (
                    <div style={{ fontSize: 12, color: colors.error }}>FallÃ³ la carga</div>
                  )}
                </div>
                <div style={{ flex: '1 1 150px' }}>
                  <FilePicker
            ...
            <div>
              ...
              {uploadStatusNuevo.imagenFile === 'error' && (
                <div style={{ fontSize: 12, color: colors.error }}>FallÃ³ la carga de la imagen</div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <ActionButton onClick={agregarVehiculo} variant="neutral" disabled={savingNuevo}>
              {savingNuevo ? 'Guardando...' : 'Agregar vehÃ­culo'}
            </ActionButton>
            <ActionButton
              onClick={() => {
                setNuevo({ marca: '', modelo: '', anio: '', color: '', patente: '', asientos: 4 });
                  ...
                  <li key={v.id} style={{ ...shared.card, alignItems: 'start' }}>
                    {v.imagenURL && (
                      <div style={{ flex: '0 0 160px' }}>
                        <img src={v.imagenURL} alt="VehÃ­culo" style={shared.image} />
                      </div>
                    )}
                        ...
                        <>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                              <div style={{ fontWeight: 500 }}>Foto del vehÃ­culo</div>
                              <FilePicker
                                label=""
                                accept="image/*"
                            ...
                            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                              {[
                                { key: 'cedulaFile', label: 'CÃ©dula de automotor' },
