...
const { isLoaded } = useJsApiLoader(MAP_LOADER_OPTIONS);
  ...
  const { loading: loadingProfile, canReserve } =
    useTravelerProfileComplete(usuario?.uid);
  ...
  useEffect(() => {
    ...
    if (origenCoords && destinoCoords) {
      ...
      setRutaError("âš ï¸ No hay coordenadas guardadas para este viaje.");
    }
  }, [isLoaded, viaje]);
  ...
  const renderEstrellas = (rep) => {
    ...
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
        <span>{"â­".repeat(llenas)}{mitad ? "âœ¬" : ""}</span>
        <span style={{ color: "#888", marginLeft: 4, fontSize: 12 }}>
          {rep.toFixed(1)} Â· {totalOpiniones}
        </span>
      </span>
    );
  };
  ...
  const handleConfirmarReserva = async () => {
    ...
    if (loadingProfile) return alert("Esperá a que cargue tu perfil.");
    if (!canReserve) { alert("Completá tu perfil."); return window.location.href = "/perfil-viajero"; }
