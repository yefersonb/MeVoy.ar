...
export default function NuevoViaje() {
  ...
  // GeolocalizaciÃ³n para origen (opcional)
  useEffect(() => {
      ...
      navigator.geolocation.getCurrentPosition(
        async ({ coords }) => {
          ...
          try {
            ...
            let raw = json.results?.[0]?.formatted_address || "UbicaciÃ³n detectada";
            ...
            setOrigen({
              formatted_address: "UbicaciÃ³n detectada",
              geometry: { location: { lat: () => coords.latitude, lng: () => coords.longitude } },
            });
          }
        },
  ...
  useEffect(() => {
    const loadVehiculos = async () => {
      ...
      try {
        ...
        console.error("Error cargando vehÃ­culos para nuevo viaje:", e);
      } finally {
  ...
  const publicarViaje = async () => {
    if (!auth.currentUser) { alert("âš ï¸ IniciÃ¡ sesiÃ³n para publicar."); return; }
    if (!origen || !destino || !fecha || !hora) {
      alert("CompletÃ¡ todos los campos obligatorios."); return;
    }
    if (!vehiculoSeleccionado) { alert("SeleccionÃ¡ un vehÃ­culo para el viaje."); return; }

    // Validaciones de paquetes cuando aplica
    if (aceptaPaquetes) {
      if (!pesoMax || !volumenMax || !costoBasePaquete) {
        alert("CompletÃ¡ peso mÃ¡ximo, volumen mÃ¡ximo y costo base de paquete."); return;
      }
    ...
    try {
      ...
      if (!snap.exists()) { alert("Usuario no registrado. CompletÃ¡ tu perfil."); return; }
      const u = snap.data();

      if (!u.nombre && !auth.currentUser.displayName) {
        alert("CompletÃ¡ tu nombre en el perfil antes de publicar.");
        return;
      }
      if (!u.whatsapp) {
        alert("AgregÃ¡ tu WhatsApp en el perfil antes de publicar.");
        return;
      }
      ...
      await addDoc(collection(db, "viajes"), {
        ...
        // ðŸ‘‡ NUEVO: configuraciÃ³n de paquetes
        aceptaPaquetes,
      ...
      console.error(e);
      alert("Error al publicar: " + (e.message || "OcurriÃ³ un problema."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section style={{ padding: "1rem" }}>
      ...
      <div style={{ margin: "1rem 0", padding: "12px", border: "1px solid #e5e7eb", borderRadius: 8 }}>
        ...
        {aceptaPaquetes && (
          <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12 }}>
            <div>
              <label style={{ display: "block", fontWeight: 500 }}>Peso mÃ¡x. (kg)</label>
              <input
            ...
            <div>
              <label style={{ display: "block", fontWeight: 500 }}>Volumen mÃ¡x. (L)</label>
              <input
      ...
      {/* VehÃ­culo */}
      <div style={{ margin: "0.5rem 0" }}>
        <div style={{ fontWeight: 500, marginBottom: 6 }}>VehÃ­culo</div>
        {vehiculosCargando ? (
          <div style={{ color: "#555" }}>Cargando vehÃ­culos...</div>
        ) : vehiculos.length === 0 ? (
          <div style={{ color: "#c0392b" }}>
            No tenÃ©s vehÃ­culos cargados. AgregÃ¡ uno en la pestaÃ±a <strong>VehÃ­culos</strong>.
          </div>
        ) : (
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <select
              aria-label="Seleccionar vehÃ­culo"
            ...
            >
              <option value="" disabled>SeleccionÃ¡ vehÃ­culo</option>
              {vehiculos.map((v) => (
